<?php
  ...
  function get_ext_only($path) {
    ...
    // webpages root:
    $htmlRoot = input_sanitized('html-root','');
...
//  if ($ix != '1') $delay = '1';
  ...
  $audioName = '';
  //
    $flac     = input_sanitized("flac", "", true, true);
    $flacName = get_name_only($flac);
    if ((empty($audioName)) && (!empty($flacName))) {
        $audioName = $flacName;
    }
    //
    $m4a      = input_sanitized("m4a2",  "", true, true);
    $m4aName  = get_name_only($m4a);
    if ((empty($audioName)) && (!empty($m4aName))) {
        $audioName = $m4aName;
    }
    //
    $oga      = input_sanitized("oga",  "", true, true);
    $ogaName  = get_name_only($oga);
    if ((empty($audioName)) && (!empty($ogaName))) {
        $audioName = $ogaName;
    }
    //
    $mp3      = input_sanitized("mp3",  "", true, true);
    $mp3Name  = get_name_only($mp3);
    if ((empty($audioName)) && (!empty($mp3Name))) {
        $audioName = $mp3Name;
    }
    //
    if (empty($audioName)) $audioName = "lydklippet";
  //
  ...
  $sheetName = get_name_only($sheet, true);
    ...
    // Maybe chords:
    $chords = input_sanitized('chords');
    $chordsName = get_name_only($chords, true);
...
>
    <img src="<?php echo "$htmlRoot"; ?>pix/media-types/audio.gif" alt="audio icon" />
    Afspil:
...
<?php if (($delay) || (empty($supplied))) echo ' style="display:none;" ' ?>
>
    <td><img src="<?php echo "$htmlRoot"; ?>pix/media-types/audio.gif" alt="audio icon" />&nbsp;</td>
    <td>
...
<?php if (empty($flac)) echo "<!--\n"; ?>
<div>
    <img src="<?php echo "$htmlRoot"; ?>pix/media-types/download-gray.png" alt="download icon" />
    Download
    <?php
        echo "<a href=\"$flac\" $save_as";
        echo ">$flacName</a>";
    ?>.
</div>
<?php if (empty($flac)) echo "-->\n"; ?>
...
<div>
    <img src="<?php echo "$htmlRoot"; ?>pix/media-types/youtube.gif" alt="youtube icon" />
    Afspil:
    <?php
        echo "<a ";
...
<div>
    <img src="<?php echo "$htmlRoot"; ?>pix/media-types/txt.gif" alt="text icon" />
    Læs nærmere
    <?php
        echo "<a ";
        echo " href=\"$about\"";
...
<div>
    <img src="<?php echo "$htmlRoot"; ?>pix/media-types/notes.gif" alt="nodeark icon" />
<?php
...
<?php if (empty($chords)) echo "<!--\n"; ?>
<div>
    <img src="<?php echo "$htmlRoot"; ?>pix/media-types/chords.gif" alt="akkord icon" />
<?php
    $bPDF = (!empty($chords) && (get_ext_only("$chords") == ".pdf"));
    if ($bPDF)
        echo "Download <a href='$chords' download title=\"Åbner ''gem som'' dialog.\">";
    else
        echo "Se <a href='$sheet' $new_tab>";
    echo "becifringer</a> her.";
?>
</div>
<?php if (empty($chords)) echo "-->\n"; ?>
...
<div>
    <img src="<?php echo "$htmlRoot"; ?>pix/media-types/html.png" alt="link icon" />
    Se den
    <a href=<?php
        echo "\"$front\"";
    ...
    <img
        <?php
            switch (get_ext_only($thoughts)) {
                case '.pdf':
                    echo 'src="' . $htmlRoot . 'pix/media-types/pdf.gif" alt="pdf icon"';
                    break;
                case '.html':
                    echo 'src="' . $htmlRoot . 'pix/media-types/html.png" alt="link icon"';
                    break;
                default:
                    echo 'src="' . $htmlRoot . 'pix/media-types/txt.gif" alt="tekst icon"';
                    break;
            }
        ?>
    />
...
<?php if (empty($supplied)) echo "/* -- No audio supplied.\n"; ?>
        ...
        var dollarWrapper = $('#id_my_jplayer_wrapper_<?php echo $ix; ?>')
        dollarWrapper.show()
        // Hide the Play link:
        var dollarPlayer = $('#id_show_jplayer_<?php echo $ix; ?>')
        dollarPlayer.hide()
...
<?php
    $supplied2 = "";
    if (!empty($flac)) {
        if (!empty($supplied2)) $supplied2 = $supplied2 . ",\n";
        $supplied2 = $supplied2 . "flac: '$flac'";
    }
    if (!empty($m4a)) {
        if (!empty($supplied2)) $supplied2 = $supplied2 . ",\n";
        $supplied2 = $supplied2 . "m4a: '$m4a'";
    }
    if (!empty($oga)) {
        if (!empty($supplied2)) $supplied2 = $supplied2 . ",\n";
        $supplied2 = $supplied2 . "oga: '$oga'";
    }
    if (!empty($mp3)) {
        if (!empty($supplied2)) $supplied2 = $supplied2 . ",\n";
        $supplied2 = $supplied2 . "mp3: '$mp3'";
    }
    if (!empty($supplied2)) echo "$supplied2\n";
?>
                });
...
<?php
    if (!$delay) {
        echo "// -- This player will never be hidden.\n";
    }
    else {
        echo "// Destroy and hide player:\n\n";
        echo "// Kill content:\n";
        echo "var dollarPlayer = $('#jquery_jplayer_" . $ix . "')\n";
        echo "dollarPlayer.jPlayer('destroy')\n";
        echo "// Make the Play link visible:\n";
        echo "var dollarShow = $('#id_show_jplayer_" . $ix . "')\n";
        echo "dollarShow.show()";
        echo "// Hide the jplayer divs:\n";
        echo "var dollarWrapper = $('#id_my_jplayer_wrapper_" . $ix . "')\n";
        echo "dollarWrapper.hide()\n";
    }
?>
    } // << end of hide
} // my_jplayer_<?php echo $ix; ?>_show_hide(...)

//document.title = 'hertil'

<?php
    // Define general function for show/hide of my_jplayer
    if ($ix == 1) {
