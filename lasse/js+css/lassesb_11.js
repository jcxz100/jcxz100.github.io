...
function LoadJPlayer(sMp3File, bKeepVisible) {
	...
	if (iIx == 0) {
		...
		var strPath = GetLocRoot() + 'Musik/jplayer_include.html #idToLoad'
		//alert(strPath)
		$(sIdDiv2).load(strPath, function(responseText, textStatus){
			if (textStatus == "success") {
				if (bKeepVisible) {
					if (IsTinyScreen()) {
				...
				$(sIdDiv2).html('error loading jplayer_include.html<br>try reloading page')
			}
		})
	}
	else {
